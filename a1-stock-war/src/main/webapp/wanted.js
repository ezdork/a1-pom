...
function appendCurrentBuyTable(tableId, data) {
	var msg = '持有股票';
	$('#content').append(
			'<table id="' + tableId + '" ><THEAD><tr><td colspan="7">' + msg
					+ '</td></tr><tr><td>股票代號</td><td>5日均</td><td>跌停價</td><td>張數</td><td>購買日期</td><td>購買價</td><td>現價</td></tr></THEAD></table>');
	var $table = $('#' + tableId);
	var list = data[tableId];
	var length = list.length;
	$table.append('<TBODY>');
	for ( var i = 0; i < length; i++) {
		var html = list[i]['nowPrice'] > list[i]['buyPrice'] ? '<tr style="color:red">' : '<tr style="color:green">';
